...
public:
  WalkAST(BugReporter &br, AnalysisDeclContext* ac,
          const ChecksFilter &f, llvm::StringSet<> wf)
...
void WalkAST::checkCall_custom(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  PathDiagnosticLocation CELoc =
    PathDiagnosticLocation::createBegin(CE, BR.getSourceManager(), AC);
  BR.EmitBasicReport(AC->getDecl(), filter.checkName_getpw,
                     "User-provided function to be warned about",
                     "Security",
                     Msg,
                     CELoc, CE->getCallee()->getSourceRange());
}

// TODO: check the correct CWE number
//===----------------------------------------------------------------------===//
...
void WalkAST::checkCall_lstrcatA(const CallExpr *CE, const FunctionDecl *FD) {
  ...
  if (!FPT->getParamType(1)->getAs<PointerType>()->getPointeeType().isConstQualified())
    return;
  ...
  if (!PTReturnType)
    return;
  if (PTReturnType->getPointeeType().getUnqualifiedType() != BR.getContext().CharTy)
    return;
...
void ento::registerSecuritySyntaxChecker(CheckerManager &mgr) {
  ...
  StringRef WarnOption = mgr.getAnalyzerOptions().getCheckerStringOption(checker, "Warn");
